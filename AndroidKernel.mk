...
#           (C) 2022 MeizuCustoms enthusiasts
#
...
# TODO: Do not use system's nproc
...
ifneq ($(TARGET_KERNEL_CLANG_COMPILE),false)
    ...
    ifeq ($(KERNEL_SD_LLVM_SUPPORT), true)
        ...
        $(warning "Using Snapdragon Clang" $(TARGET_KERNEL_CLANG_PATH))
    else
        ...
        ifneq ($(TARGET_KERNEL_CLANG_VERSION),)
            $(warning "Using custom Clang" $(TARGET_KERNEL_CLANG_PATH))
        else
            $(warning "Using AOSP Clang" $(TARGET_KERNEL_CLANG_PATH))
        endif
    endif
...
$(KERNEL_SYMLINK): $(KERNEL_OUT)
	ln -sf $(KERNEL_OUT) $(KERNEL_SYMLINK)

$(KERNEL_CONFIG): $(KERNEL_SYMLINK) $(ALL_KERNEL_DEFCONFIG_SRCS)
	@echo "Building Kernel Config"
	$(call make-kernel-config,$(KERNEL_OUT),$(KERNEL_DEFCONFIG))

$(TARGET_PREBUILT_INT_KERNEL): $(KERNEL_CONFIG) $(DEPMOD) $(DTC) $(TARGET_KERNEL_ADDITIONAL_DEPENDENCIES)
...
# For compatibility with CAF build system
$(TARGET_PREBUILT_KERNEL): $(TARGET_PREBUILT_INT_KERNEL)
	$(hide) cp $(TARGET_PREBUILT_INT_KERNEL) $(TARGET_PREBUILT_KERNEL)
...
$(BOARD_PREBUILT_DTBOIMAGE): $(DTC) $(MKDTIMG) $(MKDTBOIMG) $(TARGET_KERNEL_ADDITIONAL_DEPENDENCIES)
...
$(INSTALLED_DTBIMAGE_TARGET): $(DTC) $(TARGET_KERNEL_ADDITIONAL_DEPENDENCIES)
...
$(RECOVERY_KERNEL_SYMLINK): $(RECOVERY_KERNEL_OUT)
	ln -sf $(RECOVERY_KERNEL_OUT) $(RECOVERY_KERNEL_SYMLINK)
...
$(TARGET_PREBUILT_INT_RECOVERY_KERNEL): $(RECOVERY_KERNEL_CONFIG) $(DEPMOD) $(DTC) $(TARGET_KERNEL_ADDITIONAL_DEPENDENCIES)
